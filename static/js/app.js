...
// Раньше локаль грузилась с устаревшего npmcdn.com и иногда не успевала —
// календарь откатывался на английский с воскресеньем в начале недели.
const FP_RU = {
  weekdays: {
...
(function initQuickbook() {
  try {
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      text.innerHTML = `Быстрое бронирование для <b>${_quickbook.guest_name || 'гостя'}</b> · ${fmtDate(_quickbook.check_in)} — ${fmtDate(_quickbook.check_out)} — выберите объект`;
...
function showBookingSuccessPanel(guestName, ci, co) {
  ...
  document.getElementById('booking-success-name').textContent = guestName || 'гостя';
  document.getElementById('booking-success-info').textContent =
    `${guestName} · ${fmtDate(ci)} — ${fmtDate(co)}`;
}
...
async function openBookingModal(c) {
  // Сброс к форме (если до этого было показано сообщение об успехе)
  ...
  // Сброс пикеров дат оплаты
  if (fpDepositDate) fpDepositDate.clear();
  if (fpBalanceDate) fpBalanceDate.clear();

  // Предзаполнение данных гостя из быстрого бронирования
  if (_quickbook) {
    ...
    if (document.getElementById('booking-early-checkin'))
      document.getElementById('booking-early-checkin').checked = !!_quickbook.early_checkin;
    if (document.getElementById('booking-late-checkout'))
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.getElementById('booking-deposit-date'))
    fpDepositDate = flatpickr('#booking-deposit-date', pdOpts);
  if (document.getElementById('booking-balance-date'))
...
function initBookingCalendars() {
  ...
  fpCheckin  = flatpickr('#booking-checkin',  { ...base, disable: disableIn, minDate: 'today',
    onChange: (sel) => { if (sel[0]) { const m = new Date(sel[0]); m.setDate(m.getDate() + 1); fpCheckout.set('minDate', m); } calcBookingTotal(); } });
...
function calcBookingTotal() {
  ...
  const earlyLate   = Math.round(bookingPrice / 2) * ((earlyChk ? 1 : 0) + (lateChk ? 1 : 0));
